 ...
 * @return mixed
 *
 * @uses uw_credit_user_list_table_balance_label to control the label on the user list table for balance.
 */
function uwcs_modify_user_table( $column ) {
	$column['credits'] = apply_filters('uw_credit_user_list_table_balance_label', 'Credit Balance');
...
function uwcs_modify_user_table_row( $val, $column_name, $user_id ) {
	$user_balance = wc_price( get_user_meta( $user_id, '_uw_balance', true ) );
	switch ( $column_name ) {
		case 'credits' :
			return $user_balance;
			break;
...
/**
 * Add Credit Balance to the admin toolbar
 *
 * @uses uw_credit_user_credit for text for menu
 * @uses uw_credit_admin_bar_placement for placement of the menu.
 * @uses uw_credit_admin_bar_placement_priority for priority on admin bar action
 */
add_action( 'admin_bar_menu', 'wuwp_display_wallet_adminbar', apply_filters( 'uw_credit_admin_bar_placement_priority', 99 ) );
function wuwp_display_wallet_adminbar( $adminbar ) {
	if ( is_user_logged_in() ) {
		$wallet_balance              = wc_price( get_user_meta( get_current_user_id(), '_uw_balance', true ) );
		$uw_adminbar_ammount_content = apply_filters( 'uw_credit_user_credit', 'Your Balance: ' . $wallet_balance );
		$adminbar->add_node( array(
			'id'     => 'wuwp_wallet_ammount',
			'title'  => $uw_adminbar_ammount_content,
			'parent' => apply_filters( 'uw_credit_admin_bar_placement', 'top-secondary' )
		) );
	}
}